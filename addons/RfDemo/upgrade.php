...
// 获取模块信息
if(!($addon = \common\models\sys\Addons::findByName('RfDemo')))
{
    return false;
}
...
function updateDatabaseField($version)
{
    switch ($version)
    {
        case '1.0.1' :
            // 增加测试 - 冗余的字段
            $sql = "ALTER TABLE rf_addon_demo_curd ADD COLUMN redundancy_field varchar(48) CHARACTER SET utf8 COLLATE utf8_general_ci COMMENT '冗余字段';";
            Yii::$app->getDb()->createCommand($sql)->execute();
            break;

        case '1.0.2' :
            // 删除测试 - 冗余的字段
            $sql = "ALTER TABLE `rf_addon_demo_curd` DROP `redundancy_field`;";
            Yii::$app->getDb()->createCommand($sql)->execute();
            break;
    }
