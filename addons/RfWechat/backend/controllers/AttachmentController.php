...
use addons\RfWechat\backend\forms\PreviewForm;
use addons\RfWechat\backend\forms\SendForm;
...
class AttachmentController extends BaseController
{
    ...
    public function actionNewsEdit()
    {
        ...
        if ($request->isAjax) {
            ...
            try {
                ...
                Yii::$app->wechatServices->attachment->editNews($attachment, $list, $isNewRecord);
                $transaction->commit();
        ...
        return $this->render('news-edit', [
            ...
            'list' => Yii::$app->wechatServices->attachmentNews->formattingList($attach_id),
        ]);
    }
    ...
    public function actionCreate($type)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->local_url) {
            try {
                $res = Yii::$app->wechatServices->attachment->saveCreate($model);
    ...
    public function actionPreview($attach_id, $mediaType)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            try {
                Yii::$app->wechatServices->attachment->preview($attach_id, $model->type, $model->content);
    ...
    public function actionSync($type, $offset = 0, $count = 20)
    {
        // 查找素材
        try {
            $res = Yii::$app->wechatServices->attachment->sync($type, $offset, $count);
            if (is_array($res)) {
