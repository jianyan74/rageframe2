...
class FansController extends BaseController
{
    ...
    public function actionIndex()
    {
        ...
        return $this->render('index', [
            ...
            'keyword' => $keyword,
            'tag_id' => $tag_id,
            'all_fans' => Yii::$app->wechatServices->fans->getCountFollow(),
            'fansTags' => $tags,
    ...
    public function actionSendMessage($openid)
    {
        if (Yii::$app->request->isPost) {
            ...
            try {
                ...
                Yii::$app->wechatServices->message->customer($openid, $data['type'], $media_id);
        ...
        return $this->renderAjax('send-message', [
            'model' => Yii::$app->wechatServices->fans->findByOpenId($openid)
        ]);
    }
    ...
    {
        $fans = Yii::$app->wechatServices->fans->findByIdWithTag($fan_id);
    ...
    public function actionSyncAllOpenid()
    {
        ...
        try {
            list($total, $count, $next_openid) = Yii::$app->wechatServices->fans->syncAllOpenid();
    ...
    public function actionSync()
    {
        ...
        // 全部同步
        if ($type == 'all' && !empty($models = Yii::$app->wechatServices->fans->getFollowListByPage($page))) {
            // 同步粉丝信息
            foreach ($models as $fans) {
                Yii::$app->wechatServices->fans->syncByOpenid($fans['openid']);
            }
        ...
        if ($type == 'check') {
            ...
            // 系统内的粉丝
            if (!empty($sync_fans = Yii::$app->wechatServices->fans->getListByOpenids($openids))) {
                // 同步粉丝信息
