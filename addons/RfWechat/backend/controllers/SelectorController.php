...
class SelectorController extends BaseController
{
    ...
    public function actionList($json = false)
    {
        ...
        if ($media_type == Attachment::TYPE_NEWS) {
            $models = Yii::$app->wechatServices->attachmentNews->getFirstListPage($year, $month, $keyword);
        } else {
            $models = Yii::$app->wechatServices->attachment->getListPage($media_type, $year, $month, $keyword);
        }
