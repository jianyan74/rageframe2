...
use addons\RfWechat\common\models\RuleKeywordStat;
...
class StatController extends BaseController
{
    ...
    {
        // 更新微信统计数据
        Yii::$app->wechatServices->fansStat->getFansStat();

        $request = Yii::$app->request;
        $from_date = $request->get('from_date', date('Y-m-d', strtotime("-6 day")));
        $to_date = $request->get('to_date', date('Y-m-d'));

        $models = Yii::$app->wechatServices->fansStat->findBetweenByCreatedAt($from_date, $to_date);
        ...
        if ($yesterdayModel = Yii::$app->wechatServices->fansStat->findByCreatedAt(strtotime(date('Y-m-d')) - 60 * 60 * 24)) {
        ...
        if ($todayModel = Yii::$app->wechatServices->fansStat->findByCreatedAt(strtotime(date('Y-m-d')))) {
