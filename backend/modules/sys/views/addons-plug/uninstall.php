...
<script type="text/html" id="listModel">
    ...
    <tr id = "{{value.id}}">
        ...
        <td>
            <a href="{{value.upgradeConfigUrl}}" onclick="rfTwiceAffirm(this, '确认更新配置吗？', '会重载最新模块的配置');return false;"><span class="btn btn-info btn-sm">更新配置</span></a>
            <a href="{{value.upgradeUrl}}" onclick="rfTwiceAffirm(this, '确认更新数据吗？', '会执行更新数据库字段升级等功能');return false;"><span class="btn btn-info btn-sm">更新数据</span></a>
            <a href="{{value.ajaxEditUrl}}"><span class="btn btn-info btn-sm">更新数据</span></a>
            {{if value.status == 0 }}
