...
<?php foreach($models as $k => $model){ ?>
    <tr id="<?php echo $model['id']?>" class="<?php echo $pid?>">
        ...
        <td><div class="fa <?php echo $model['menu_css']?>"></div></td>
        <td>
        ...
        <td>
            <a href="<?php echo Url::to(['edit','id' => $model['id'], 'cate_id' => $cate_id, 'parent_title' => $parent_title, 'level' => $model['level']])?>" data-toggle='modal' data-target='#ajaxModal'>
                <span class="btn btn-info btn-sm">编辑</span>
            </a>
            <?php echo \common\helpers\HtmlHelper::statusSpan($model['status']); ?>
            <a href="<?php echo Url::to(['delete','id' => $model['id'], 'cate_id' => $cate_id])?>"  onclick="rfDelete(this);return false;">
