    ...
    <div class="file-manager">
        <?php if($addon['is_setting'] == StatusEnum::ENABLED || !empty(Yii::$app->params['addonBinding']['cover']) || $addon['is_rule'] == StatusEnum::ENABLED){ ?>
            ...
            <ul class="folder-list p-xs">
                <?php if(!empty(Yii::$app->params['addonBinding']['cover'])){ ?>
                    <li>
                        <a href="<?php echo Url::to(['/addons/cover', 'addon' => Yii::$app->params['addon']['name']])?>" title="应用入口">
                ...
                <?php if($addon['is_rule'] == StatusEnum::ENABLED){ ?>
                    <li>
                        <a href="<?php echo Url::to(['/addons/rule', 'addon' => Yii::$app->params['addon']['name']])?>" title="规则管理">
                ...
                <?php if($addon['is_setting'] == StatusEnum::ENABLED){ ?>
                    <li>
                        <a href="<?php echo AddonUrl::to(['setting/display', 'addon' => Yii::$app->params['addon']['name']])?>" title="参数设置">
        ...
        <ul class="folder-list p-xs">
            <?php foreach (Yii::$app->params['addonBinding']['menu'] as $vo){ ?>
                <li>
                    <a href="<?php echo AddonUrl::to([$vo['route']])?>" title="<?php echo $vo['title'] ?>">
                        <i class="<?php echo $vo['icon'] ? $vo['icon'] : 'fa fa-puzzle-piece'; ?>"></i><?php echo $vo['title'] ?>
                    </a>
                </li>
            <?php } ?>
        </ul>
