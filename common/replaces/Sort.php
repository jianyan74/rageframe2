...
class Sort extends BaseObject
{
    ...
    public function createUrl($attribute, $absolute = false)
    {
        ...
        foreach ($route as $key => $item) {
            if ($key == 0) {
                $newParams[$key] = $item;
                $newParams[1] = StringHelper::toUnderScore(Yii::$app->params['addonInfo']['name']);
            } else {
                $newParams[$key + 1] = $item;
            }
        }

        $params[0] = implode('/', $newParams);


        $urlManager = $this->urlManager === null ? Yii::$app->getUrlManager() : $this->urlManager;
        if ($absolute) {
            return urldecode($urlManager->createAbsoluteUrl($params));
        }

        return urldecode($urlManager->createUrl($params));
    }
