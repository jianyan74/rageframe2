 ...
 */
class AuthService extends \common\components\Service
{
    /**
     * 当前用户权限
     *
    ...
    public function getAllAuthByRole()
    {
        $role = $this->getRole();
        // 获取缓存
        if (!($auth = Yii::$app->cache->get('allAuth' . $role['name'])))
        {
            ...
            Yii::$app->cache->set('allAuth' . $role['name'], $auth, 3600, $dependency);
        }
